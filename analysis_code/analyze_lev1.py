...
def get_confounds_tedana(confounds_file, task):
    ...
    # else:
    confounds = confounds_df.filter(regex='cosine|a_comp_cor_0[0-4]|framewise_displacement' 
                                    '|trans_x$|trans_x_derivative1$'
                                    '|trans_y$|trans_y_derivative1$'
                                    '|trans_z$|trans_z_derivative1$'
                                    '|rot_x$|rot_x_derivative1$'
...
def make_desmat_contrasts(root, task, events_file, duration_choice, 
...
def check_file(glob_out, task):
    ...
    """
    if task in ['cuedTS', 'directedForgetting', 'flanker', 'goNogo',
...
def get_files(root, subid, task):
    ...
    files['events_file'] = sorted(glob.glob(
        f'{root}/sub-{subid}/ses-*/func/*{task}_*events*tsv'
    ))
    
    files['confounds_file'] =  sorted(glob.glob(
        f'{root}/sub-{subid}/ses-*/func/*{task}_*confounds*.tsv'
    ))

    files['mask_file'] = sorted(glob.glob(
        f'{root}/sub-{subid}/ses-*/func/*{task}_*mask*.nii.gz'
    ))
    
    files['data_file'] = sorted(glob.glob(
    f'{root}/sub-{subid}/ses-*/func/*{task}_*_bold.nii.gz'
    ))
...
def get_parser():
    ...
    parser.add_argument(
        'task',
        choices=['cuedTS', 'directedForgetting', 'flanker', 'goNogo',
                'nBack', 'stopSignal', 'spatialTS', 'shapeMatching',
    ...
    parser.add_argument(
        '--fixed_effects',
        action='store_true',
        help=('Use this flag to run fixed effects analysis within subject across sessions.')
    )
    parser.add_argument(
        '--simplified_events',
        action='store_true', 
...
if __name__ == "__main__":
    ...
    assert len(files['events_file']) == total_num_files & len(files['confounds_file']) == total_num_files & len(files['events_file']) == total_num_files
    effect_size_files = {}
    variance_files = {}
    for data_file in files['data_file']:
        ...
        design_matrix, contrasts, tr, percent_junk, events_df = make_desmat_contrasts(root, task, 
            event_file, duration_choice, 
            add_deriv, n_scans, confounds_file, regress_rt
        )
        ...
        exclusion, any_fail = qa_design_matrix(contrast_dir, contrasts, design_matrix, subid, task,ses, percent_junk = percent_junk)

        add_to_html_summary(subid, contrasts, design_matrix, contrast_dir, regress_rt, duration_choice, task, any_fail, exclusion, ses, percent_junk)
        ...
        if not any_fail and qa_only == False:
            ...
            if not residuals:
                ...
                for con_name, con in contrasts.items():
                    ...
                    effect_size_filename = (f'{contrast_dir}/contrast_estimates/sub-{subid}_{ses}_task-{task}_contrast-{con_name}'
                        f'_rtmodel-{regress_rt}_stat'
                        f'-effect-size.nii.gz')
                    con_est['effect_size'].to_filename(effect_size_filename)
                    variance_filename = (f'{contrast_dir}/contrast_estimates/sub-{subid}_{ses}_task-{task}_contrast-{con_name}'
                        f'_rtmodel-{regress_rt}_stat'
                        f'-variance.nii.gz')
                    con_est['effect_variance'].to_filename(variance_filename)
                    zscore_filename = (f'{contrast_dir}/contrast_estimates/sub-{subid}_{ses}_task-{task}_contrast-{con_name}'
                        f'_rtmodel-{regress_rt}_stat'
                        f'-z_score.nii.gz')
